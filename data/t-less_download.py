...
def parse():
    parser = argparse.ArgumentParser(
        description="Downloads and unpacks the selected parts of the T-LESS dataset. "
                    "Example usage: "
                    "\"t-less_download.py --train --sensors primesense\" "
...
def main():
    global step_total
    path_dest_home, ids_lists, sensors = parse()
    step_total = len(sensors) * len(ids_lists) * sum((len(i) for i in ids_lists))
